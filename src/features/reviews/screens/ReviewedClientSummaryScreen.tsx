    ...
    <View className="flex-1 bg-background">
      ...
      <Container variant="narrow" safeArea="bottom" scroll>
        <View className="px-6 py-6 gap-8">
          ...
          {!isLoading && summary.data && hasReviews && (
            <>
              {/* Average rating — stars lead since they're the shape a
                  glance reads fastest, with the exact number right below
                  for precision. A 4.2 rating and a 1.2 rating shouldn't
                  look identical here, which a single number alone would. */}
              <View className="items-center gap-2">
                <StarRating value={summary.data.average_rating} size={36} />
              ...
              {/* Tag summary — a single card, header row on top and data
                  rows below it. Each vote count renders as a small
                  badge rather than plain text, sorted by count so the
                  most-picked attributes surface first. */}
              {sortedTags.length > 0 && (
                <View className="gap-3 mt-4">
                  <Text variant="overline" className="px-4">
                    What reviewers said
                  </Text>
                    ...
                    <View className="gap-2">
                      {sortedTags.map((tag: TagSummaryItem) => (
                        ...
                        >
                          ...
                          <View className="min-w-[28px] items-center rounded-md bg-secondary px-2 py-0.5">
